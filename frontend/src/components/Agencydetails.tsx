...
const JobSeekers: React.FC = () => {
  ...
  function chatClickHandler(jobseekerId: string) {
    navigate(`/chat/${jobseekerId}`);
  }
  ...
  const handleStatusUpdate = async (id: string, status: string) => {
    ...
    try {
      await axios.put(
        `http://localhost:4000/users/jobseekers/${id}`,
        { status },
        {
          headers: {
            Authorization: `Bearer ${token}`,
            ...
            <div>
              ...
              <div className="mt-2">
                ...
                >
                  ...
                  </div>) : jobSeeker.status === "accepted" ? (<button onClick={() =>{chatClickHandler(jobSeeker.id)}}>Join Chat</button>) : ("Status")
                } */}

                {
                  jobSeeker.status === "accepted" ? 
                  (<button onClick={() =>{chatClickHandler(jobSeeker.id)}}>Join Chat</button>) :
                  (
                    <>
                    <button
                      className="btn btn-success me-2"
